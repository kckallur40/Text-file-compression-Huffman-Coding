...
// The main function that builds a Huffman Tree and print codes by traversing the Huffman Tree
...
int main()
{
	...
	cout<<"\t====================================";
	cout<<"\n\nEnter the name of text file to compress : ";
	cin>>filename;
	...
	cout<<"\n\tCharacter\t\tHuffman Code\n";
   	...
   	HuffmanCodes(ASCII_values,actual_frequency,count1);  //build Huffman tree
	...
	//printTree(cout,minHeap.top(),0);      //display formed Tree  with '■' as parent nodes (including root)
	write_codes();
	ofstream fout;
  	fout.open(filename,ios::binary);

	cal_compressionRatio();                //calculate compression ratio
	int temp1;
	BitwiseWrite s(fout);
	cout<<"\nProcessing.... Plz wait !!!";
	for(int d = 0;d < Huffcode.size();d++) {
	    temp1 = Huffcode[d];
	    s.writeBit(temp1);                //write Bit by bit by bit in compressed file
	}
